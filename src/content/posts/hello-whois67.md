---
title: Hello, whois67
published: 2026-08-02
description: 建站第一篇：从 Firefly 模板到自己的小站。
tags: [建站, 随笔]
category: Talk is cheap
draft: false
pinned: true
comment: true
---

这是 **whois67** 博客的第一篇文章。

## 这个站是什么

个人小站，用来记一点技术折腾、影视和阅读。目前已经接上：

- **影视**：B 站公开追番
- **阅读计划**：微信读书公开书架与阅读统计
- **关于**：站长自述

主题基于 [Firefly](https://github.com/CuteLeaf/Firefly)（Astro），内容用 Markdown 写，准备部署在 Cloudflare。

## 分类说明

之后文章会尽量归到这些分类里：

| 分类 | 大概写什么 |
|------|------------|
| Talk is cheap | 碎碎念、随笔、建站记录 |
| That tastes good | 产品、设计、审美与品味 |
| Action speaks louder | 动手实践、项目、教程 |
| Way to AGI | AI / 学习 / 技术路线 |
| You gotta eat | 吃喝、生活烟火 |
| See the world | 见闻、旅行、影视延伸 |

## 接下来

会慢慢补文章、动态，以及留言评论。欢迎来逛。

> 好好学习，天天向上。
